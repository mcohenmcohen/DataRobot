    ...
    for c in clusters:
        for fdw in fdws:
            for fd in fds:
                ...
                while n_tries_project_creation > 0:
                    try:
                        ...
                        n_tries_project_creation -= 1
                        print(f'Something went wrong during project creation: {ex}')                
                if not n_tries_project_creation:  # all tries exhausted
                    print(f'Skipping project "{project_name}", moving onto next iteration')
                    continue
