...
createTrainTest <- function(target, sample_size = 0.5){
  # dplyr required
  require(dplyr)
...
plotFeatureImpacts <- function(project, models, cutoff = 0.05, normalized = TRUE){
  ...
  ids <- which(sapply(models, function(x) !any(tryCatch(GetFeatureImpactForModel(x),
  ...
  # Creating Plot
  if(normalized){
    out_plot <- ggplot(impacts, aes(x = reorder(featureName, norm_value, median), y = norm_value)) + geom_boxplot() +
      geom_hline(aes(yintercept = cutoff, colour = "red")) + theme(legend.position="none") +
      ylab("Normalized Importance Score") + xlab("") + coord_flip()
  }else{
    ...
    unnorm_cutoff <- quantile(feature_impact_medians$unnorm_median_value, norm_quant)
    out_plot <- ggplot(impacts, aes(x = reorder(featureName, value, median), y = value)) + geom_boxplot() +
      geom_hline(aes(yintercept = unnorm_cutoff, colour = "red")) + theme(legend.position="none") +
      ylab("Unnormalized Importance Score") + xlab("") + coord_flip()
  }
...
partialDependence <- function(project, model, data, feature, size_of_grid = 25, 
  ...
  if(!is.null(sample_size)){
    ...
    if(project_info$targetType == "Regression" | !project_info$target %in% colnames(data)){
      # Random sample
      data <- data %>% sample_n(size = min(nrow(data), sample_size), replace = FALSE)
    }else{
      # Stratified random sample
      data <- data %>%
        group_by_at(vars(project_info$target)) %>%
        mutate(num_rows=n()) %>%
        sample_frac(size = min(1, sample_size/nrow(data)), weight = num_rows, replace = FALSE) %>%
        ungroup %>%
        as.data.frame()
    }
  }
  ...
  if(length(size_of_grid) == 1){
    ...
    if(length(cats) > size_of_grid){
      if(feature_type == "numeric"){
        sampled_values <- quantile(data[, feature], probs = seq(0.05, 0.95, length.out = size_of_grid), na.rm = TRUE)
      }else{
  ...
  cat("Uploading augmented dataset of size", format(object.size(augmented_dataset), units = "Mb"), 
  ...
  pred_job_id <- RequestPredictionsForDataset(project, model$modelId, augmented_dataset_id$id)
  ...
  if(project_info$targetType == "Regression"){
    augmented_dataset$predictions <- GetPredictions(project, pred_job_id,
                                                    type = "response", maxWait = 600000)
  }else{
    augmented_dataset$predictions <- GetPredictions(project, pred_job_id,
                                                    type = "probability", maxWait = 600000)
  }
  ...
  if(feature_type == "numeric"){
    pd_plot <- ggplot() +
      geom_line(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_pred, group = 1), size = 2.5, color = "black") +
      geom_line(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_pred, group = 1), size = 2, color = "gold") +
      xlab(noquote(feature)) + ylab(paste0("Target (", project_info$target, ")")) + theme_bw() +
      ggtitle("Partial dependence")
  }else{
    pd_plot <- ggplot() +
      geom_point(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_pred, group = 1), size = 4.5, color = "black") +
      geom_point(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_pred, group = 1), size = 4, color = "gold") +
      xlab(noquote(feature)) + ylab(paste0("Target (", project_info$target, ")")) + theme_bw() +
  ...
  if(ice_plot){
    if(feature_type == "numeric"){
      pd_plot$layers <- c(geom_line(data = ice_plot_data, aes(x = !!ensym(feature), y = predictions, group = rowID), 
                                    size = 1, color = "darkgray", alpha = 0.25), pd_plot$layers)
    }else{
      pd_plot$layers <- c(geom_point(data = ice_plot_data, aes(x = !!ensym(feature), y = predictions, group = rowID), 
                                     size = 1, color = "darkgray", alpha = 0.25), pd_plot$layers)
    }
  }
  ...
  if(std_dev_plot){
    if(feature_type == "numeric"){
      pd_plot <- pd_plot +
        geom_line(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_minus_sd, group = 1), size= 1, color = "darkorchid") +
        geom_line(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_plus_sd, group = 1), size = 1, color = "darkorchid")
    }else{
      pd_plot <- pd_plot + 
        geom_point(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_minus_sd, group = 1), size = 2, color = "darkorchid") +
        geom_point(data = pd_plot_data, aes(x = !!ensym(feature), y = mean_plus_sd, group = 1), size = 2, color = "darkorchid")
    }
  }
...
predictionIntervals = function(project, model, data, newdata, alpha = 0.1, locally_weighted = FALSE) {
  #Loading required package
  require(stringr)
  ...
  if(locally_weighted) {
    ...
    if(!is.null(project_info$partition$userPartitionCol)){
      mad_partition <- CreateUserPartition(validationType = project_info$partition$validationType, 
      ...
      SetTarget(project = mad_project, target = colnames(data)[target_index], mode = "manual", metric = "MAE",
    ...
    eureqa <- bps[grep("Eureqa Generalized Additive Model", sapply(bps, '[[', "modelType"))]
    # Choosing the Eureqa GAM with less generations
    chosen <- which.min(as.numeric(str_extract(sapply(eureqa, '[[', "modelType"), "\\-*\\d+\\.*\\d*")))
    # In case a model was built not on the informative features
    new_list <- CreateFeaturelist(mad_project, "new_list", ListModelFeatures(model))
    # Train on all data
    mad_model_id <- RequestNewModel(mad_project, blueprint = eureqa[[chosen]], featurelist = new_list, samplePct = 100)
    mad_model <- GetModelFromJobId(mad_project, modelJobId = mad_model_id, maxWait = 600000)
    ...
    mad_pred <- RequestPredictionsForDataset(mad_project, mad_model$modelId, mad_test$id)
  ...
  # Training coverage
  coverage <- sum(ifelse(y >= (fit - d * mad_out) & y <= (fit + d * mad_out), 1, 0))/nrow(data)
  
  # Make predictions on new data
  test <- UploadPredictionDataset(project, newdata)
  pred <- RequestPredictionsForDataset(project, model$modelId, test$id)
  ...
  cat("Coverage on training data =", coverage, "with average interval length of", 2*mean(d*mad_out), '\n')
...
findThreshold <- function(project, model, target, maximize = FALSE, TNC=-1, TPC=-1, FNC=1, FPC=1){
  # Loading required package
  require(MLmetrics)
  ...
  train_ids <- ListTrainingPredictions(project)
  if(length(train_ids) != 0){
    model_ids <- sapply(train_ids, '[[', which(names(train_ids[[1]]) == "modelId"))
    train_data_ids <- sapply(train_ids, '[[', which(names(train_ids[[1]]) == "id"))
    #If already done, extract. If not, go get them
    if(model$modelId %in% model_ids){
      id <- train_data_ids[which(model_ids == model$modelId)]
  ...
  cost_fun <- function(actuals, probs, threshold){
    levs <- levels(actuals)
    predicted_labels <- as.factor(ifelse(probs > threshold, event_class, levs[which(levs != event_class)]))
  ...
  obj_fun <- function(x) cost_fun(actuals = y, probs = fit[,paste0("class_", event_class)], threshold = x)
  out <- optimize(obj_fun, interval = c(0, 1), maximum = maximize)
  
  # Attach names
  out <- list(model_name = model$modelType, best_threshold = out[[1]], optimized_value = out$objective)
  return(out)
}
...
importanceTest <- function(project, data, model, S, workers = -1,
                           seed = 0, partition = NULL, weights = NULL, responseCap = NULL,
                           smartDownsampled = NULL, majorityDownsamplingRate = NULL, 
                           scaleoutModelingMode = NULL, offset = NULL, exposure = NULL,
  ...
  for(i in 1:S){
    ...
    null.project <- SetupProject(dataSource = data, projectName = "Feature Impact Null",
                                 maxWait = maxWait)
    SetTarget(project = null.project, target = colnames(data)[target.index], mode = "manual",
              metric = p.info$metric, targetType = p.info$targetType,
              positiveClass = p.info$positiveClass,
              partition = partition, weights = weights, seed = seed, responseCap = responseCap,
              smartDownsampled = smartDownsampled, majorityDownsamplingRate = majorityDownsamplingRate, 
              scaleoutModelingMode = scaleoutModelingMode,
              offset = offset, exposure = exposure, eventsCount = eventsCount, maxWait = maxWait)
    ...
    if(model$modelCategory == "blend"){
      ...
      models.to.blend.bps <- sapply(models.to.blend, function(x) GetModel(project, x)$blueprintId)
      f.lists <- list()
      for(j in 1:length(models.to.blend)){
        f.lists[[j]] <- CreateFeaturelist(null.project, paste0("model_", j), 
                                          ListModelFeatures(GetModel(project, models.to.blend[j])))
      }
      # Requesting null models
      model.job.ids <- rep(0, length(models.to.blend))
      for (i in 1:length(models.to.blend)){
        model.job.ids[i] <- RequestNewModel(null.project, 
                                            blueprint = list(projectId = null.project$projectId,
      ...
      null.model.id <- RequestBlender(null.project, modelIds = models.to.blend, blendMethod = m)
      null.model <- GetModelFromJobId(null.project, null.model.id)
    } else {
      # In case a model was built not on the informative features
      new.list <- CreateFeaturelist(null.project, "new.list", model.features)
      null.model.id <- RequestNewModel(null.project, blueprint = true.model, featurelist = new.list)
      null.model <- GetModelFromJobId(null.project, modelJobId = null.model.id)
    }
    null.feature.impact.id <- RequestFeatureImpact(null.model)
    null.feature.impact <- GetFeatureImpactForJobId(null.project, null.feature.impact.id)
    ...
    result[,i] <- matrix(ifelse(both$impactUnnormalized.y >= both$impactUnnormalized.x, 1, 0), ncol = 1)
