...
# 5797b01c196da75f42adc7b5 Gluster cluster

#Load data
suppressWarnings(Base <- getLeaderboard('57890084a40efd734597d926'))
suppressWarnings(Gluster <- getLeaderboard('577f0bc6d241bf71dd576884'))
Base[,test := 'Shrink']
Gluster[,test := 'Gluster']

#Combine data
dat <- rbind(Base, Gluster, fill=TRUE, use.names=TRUE)
...
dat <- dat[!is.na(Shrink) & !is.na(Gluster),]

#Debug
dat[Gluster > 1 | Shrink > 1, table(as.character(main_task))]
...
#Plot overall stats
ggplot(dat, aes(x=Shrink, y=Gluster, col=Filename)) +
  geom_point() +
  ...
  guides(col = guide_legend(nrow = 7, byrow = TRUE))
