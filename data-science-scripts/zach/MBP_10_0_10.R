...
# 5797b01c196da75f42adc7b5 MBP10 cluster

#Load data
suppressWarnings(MBP9 <- getLeaderboard('5798b271c79de05dc217486e'))
suppressWarnings(MBP10 <- getLeaderboard('579835bed1df032b32c0a3cd'))
MBP9[,test := 'MBP9']
MBP10[,test := 'MBP10']

#Combine data
dat <- rbind(MBP9, MBP10, fill=TRUE, use.names=TRUE)
...
#Plot overall stats
ggplot(dat[Sample_Pct == 64,], aes(x=MBP9, y=MBP10, col=main_task)) +
  geom_point() +
...
#Plot by model
bad_file <- dat[variable == 'model_training_time_hours' & MBP10 > 1.75*MBP9, sort(unique(Filename))]
bad_model <- dat[variable == 'model_training_time_hours' & MBP10 > 1.75*MBP9, sort(unique(main_task))]
ggplot(dat[Sample_Pct == 64 & Filename %in% bad_file & main_task %in% bad_model,], aes(x=MBP9, y=MBP10, col=main_task)) +
  geom_point() +
  geom_abline(slope=1, intercept=0,linetype="dashed") +
  theme_bw() +
  scale_colour_manual(values=colors) +
