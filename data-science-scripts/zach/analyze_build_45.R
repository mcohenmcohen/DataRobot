...
old_mbtest_ids <- c(
  '5d2c92d37347c90025303dd2'
)
new_mbtest_ids <- c(
  '5d28ab887347c9002b7e2837'
)
...
res_normal = res[variable == 'Max_RAM_GB' & sample_round==64, vars, with=F]
...
res_cat <- res_cat[,list(
  ...
  worst_RAM_model = main_task[which.max(Max_RAM_GB)],
  best_mase_model = main_task[which.min(MASE_H)]
), by=c('run', 'Filename', 'sample_round')]

measures = c('worst_RAM_model', 'best_gini_model', 'best_mase_model')
...
cat_norm = res_cat[sample_round==64 & variable == 'worst_RAM_model',]

values = c(old_release, new_release)
cat_norm = dcast.data.table(cat_norm, Filename ~ variable, value.var = values)

res_normal = merge(res_normal, cat_norm, by='Filename')[order(diff_Max_RAM_GB),][diff_Max_RAM_GB > .3,]
...
res[sample_round==64 & abs(diff) > 0.05 & variable %in% c('Gini_V'),][order(pct_diff),]
