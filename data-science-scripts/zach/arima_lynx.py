...
ts_train = pd.read_csv('/Users/zachary/datasets/lynx_train.csv')['x'].values
ts_test = pd.read_csv('/Users/zachary/datasets/lynx_test.csv')['x'].values
ts = np.hstack((ts_train, ts_test))
...
pred_test = final_model.predict(start=len(ts_train)+1, end=len(ts_train)+len(ts_test))
...
ax1.plot(ts)
ax1.plot(np.hstack((pred_train, pred_test)),label=1)
ax1.plot(np.hstack(pred_train), label=2)
