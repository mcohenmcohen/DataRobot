...
library(ggplot2)
library(ggthemes)
SEED <- sample(1e6, 1)  # Try 42, 132451345, 767071, 381743, 159534, 923453, sample(1e6, 1)
set.seed(SEED)
colors <- c(
...
# Make simulated data
A <- MixSim(MaxOmega = 0, K = N_clusters, p = 2)
sim <- simdataset(5000, A$Pi, A$Mu, A$S)
sim <- data.table(
  x=as.numeric(scale(sim$X[,1], center=T, scale=T)),
  y=as.numeric(scale(sim$X[,2], center=T, scale=T)),
  true_cluster=paste0('c', sim$id)
)
...
ggplot(plot_data, aes(x=x, y=y, color=cluster)) +
  ...
  facet_wrap(~data) + ggtitle(SEED) + coord_cartesian()
print(SEED)
