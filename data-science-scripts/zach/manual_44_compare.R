...
old_mbtest_ids <- c(
  '5b11838ff76d9b000159a8b3',
  '5b158457ddfa8200014f8eba',
  '5b17141a6805650001d042da',
  '5b1eb2d84409ca00014cf819',
  '5b22fb29218f64000116d207',
  '5b15918dab51fe00012669ba',
  '5b1929daf2842e0001e9d8c7'
  )
new_mbtest_ids <- c(
...
ggplot(res[sample_round == 64 & variable %in% plot_vars,], aes(x=`4.3`, y=`4.4`)) + 
  ...
  theme_bw() + theme_tufte() + ggtitle('4.4 vs 4.3 non time series results')

plot_vars = c('Max_RAM_GB', 'Total_Time_P1_Hours', 'MASE_V', 'MASE_H')
ggplot(res[sample_round == 0 & variable %in% plot_vars,], aes(x=`4.3`, y=`4.4`)) + 
  geom_point() + geom_abline(slope=1, intercept=0) + 
  facet_wrap(~variable, scales='free') + 
  theme_bw() + theme_tufte() + ggtitle('4.4 vs 4.3 time series results')
