...
old_mbtest_ids <- c(
  '5c6d9a119103dd0001d16ea0',
  '5c7ed73b75193c0001b18d35',
  '5c7fd3bde7bae00001c17564',
  '5c829dd658365c00012c5677',
  '5c8054acb01c760001aab2e3',
  '5c812d757305070001a2bfbb',
  '5c8d18683b055b00017a3a85',
  '5c6dc5f93b9d1b000118c690',
  '5c7594a1887c7f0001f0a92c',
  '5c7ed719995bf300012015df'
)
new_mbtest_ids <- c(
  '5cb737d27347c90026fd6560',
  '5cb61f7e53bf7d000157e57a',
  '5cbddc7a7a3b7a0001b8a0e7',
  '5cbe551a7347c900284fabcb',
  '5cbddc61cb883b000137241c',
  '5cb9d9ee7347c90024715ca4',
  '5cb62082994b230001f3e454'
)
...
#Non Ts
res[sample_round == 64 & variable %in% plot_vars & !is.na(diff) & diff>1 & variable=='Max_RAM_GB',list(Filename, v5.0.0, v5.0.M1, diff)]
res[sample_round == 6 & variable %in% plot_vars & !is.na(diff) & diff>1 & variable=='Total_Time_P1_Hours',list(Filename, v5.0.0, v5.0.M1, diff)]

#TS
res[sample_round == 0 & variable %in% plot_vars & !is.na(diff) & diff>1 & variable=='Max_RAM_GB',list(Filename, v5.0.0, v5.0.M1, diff)]
res[sample_round == 0 & variable %in% plot_vars & !is.na(diff) & diff>1 & variable=='Total_Time_P1_Hours',list(Filename, v5.0.0, v5.0.M1, diff)]
...
# RAM non-ts
res[
  sample_round == 64 & variable %in% 'Max_RAM_GB' & !is.na(diff),][which.max(diff),]

# RAM ts
res[
  sample_round == 0 & variable %in% 'Max_RAM_GB' & !is.na(diff),][which.min(diff),]
...
dat[Filename=='airbnb_train_OTP.csv' & run == old_release,][which.max(Max_RAM_GB), main_task]
dat[Filename=='airbnb_train_OTP.csv' & run == new_release,][which.max(Max_RAM_GB), main_task]

dat[Filename=='multiseries_daily_1000series_classification_train.csv' & run == old_release,][which.max(Max_RAM_GB), main_task]
dat[Filename=='multiseries_daily_1000series_classification_train.csv' & run == new_release,][which.max(Max_RAM_GB), main_task]

dat[Filename=='multiseries_daily_1000series_train.csv' & run == old_release,][which.max(Max_RAM_GB), main_task]
dat[Filename=='multiseries_daily_1000series_train.csv' & run == new_release,][which.max(Max_RAM_GB), main_task]


dat[Filename=='multiseries_daily_1000series_classification_train.csv' & run == old_release,][which.max(Total_Time_P1), main_task]
dat[Filename=='multiseries_daily_1000series_classification_train.csv' & run == new_release,][which.max(Total_Time_P1), main_task]
