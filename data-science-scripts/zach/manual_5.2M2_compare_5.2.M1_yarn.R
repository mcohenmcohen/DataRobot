...
old_mbtest_ids <- c(
  '5da4e7117a3b7a0001b2ce96',
  '5da76b2a5f01ec0001554900',
  '5da7cf9eaa15140001845bdb',
  '5da9eeda742232000c618176',
  '5da8622a539cb800016c0371',
  '5da85fd9ad93bf0001ae2688',
  '5da76f68c8aeaf000181b796',
  '5da4e70338fd2e00019a2bad'
  )
new_mbtest_ids <- c(
  '5dc2343c41c0ff0001acc8bd',
  '5dc08dcff03bfc00017a8cbc',
  '5dbe23e5c0d86e0001560fe5',
  '5dc231b6a8135c00011cfdde',
  '5dbe23e698c4f70001cca467'
)
...
# diffs - non TS
ss = 64
res[sample_round==ss & diff > 0.1 & pct_diff > 0.25 & variable %in% c('Total_Time_P1_Hours'),][order(diff),list(Filename, Y_Type, get(old_release),  get(new_release), diff)]
res[sample_round==ss & diff > .5 & pct_diff > 0.25 & variable %in% c('Max_RAM_GB'),][order(diff),list(Filename, Y_Type, get(old_release),  get(new_release), diff)]
res[sample_round==ss & diff > 0.01 & pct_diff > 0.25 & variable %in% c('Gini_H'),][order(diff),list(Filename, Y_Type, get(old_release),  get(new_release), diff)]
res[sample_round==ss & diff > 0.01 & pct_diff > 0.1 & variable %in% c('Logloss_H'),][order(diff),list(Filename, Y_Type, get(old_release),  get(new_release), diff)]
