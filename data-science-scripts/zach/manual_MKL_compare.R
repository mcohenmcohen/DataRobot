...
old_mbtest_ids <- c(
  '5db0d00612deaa000ae4c8e7'
  #'5db6dea9153a1c000c1fe966'
  )
new_mbtest_ids <- c(
  '5db321ff5eb217000cbb1d3b'
  #'5db70de7a73e73000b9240e7'
)
...
fwrite(dat_old, '~/Downloads/ATLAS_raw_data.csv')
fwrite(dat_new, '~/Downloads/MKL_raw_data.csv')
...
res = dcast.data.table(res, Filename + Y_Type + sample_round + variable + main_task ~ run, value.var='value')
...
fwrite(res[!is.na(diff),], '~/Downloads/MKL_and_ATLAS_combined_and_cleaned.csv')
...
plot_vars = c('Max_RAM_GB', 'Total_Time_P1_Hours', 'LogLoss_H', 'Gini_H')
ggplot(res[sample_round == 64 & variable %in% plot_vars & !is.na(diff),]) + 
  geom_boxplot(aes(y=pct_diff, col=Y_Type)) + geom_abline(slope=0, intercept=0) +
  facet_wrap(~variable, scales='free') + 
  theme_bw() + theme_tufte() + ggtitle(paste(test, old_release, 'vs', new_release, 'non time series results')) + scale_y_log10()
summary(res[sample_round == 64 & variable %in% c('Total_Time_P1_Hours') & !is.na(diff),])

plot_vars = c('Max_RAM_GB', 'Total_Time_P1_Hours', 'LogLoss_H', 'Gini_H')
ggplot(res[sample_round == 0 & variable %in% plot_vars & !is.na(diff),]) + 
  geom_boxplot(aes(y=pct_diff, col=Y_Type)) + geom_abline(slope=0, intercept=0) +
  facet_wrap(~variable, scales='free') + 
  theme_bw() + theme_tufte() + ggtitle(paste(test, old_release, 'vs', new_release, 'non time series results')) + scale_y_log10()
summary(res[sample_round == 0 & variable %in% c('Total_Time_P1_Hours') & !is.na(diff),])
