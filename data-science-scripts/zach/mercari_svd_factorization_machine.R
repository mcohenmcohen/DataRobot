...
# Load raw data
train_raw <- fread('https://s3.amazonaws.com/datarobot_public_datasets/mer_train_small.csv')
test_raw <- fread('https://s3.amazonaws.com/datarobot_public_datasets/mer_test_small.csv')
...
model <- cv.glmnet(train_final, log(train_target), family="gaussian", alpha=1, nfolds=NFOLD, parallel=T)
...
# Best Kaggle: .37.  DR: .45
rmsle(train_target, exp(p_train))  # 0.4661501
rmsle(test_target, exp(p_test))  # 0.4967203
