...
old_release <- 'old'
new_release <- 'new'
test <- 'Overbooking' #   Docker or YARN or RPM
TIME_SERIES <- FALSE

#new_mbtest_ids <- c('5e266c2e29b2ac000c5759a2', '5e266c4629b2ac000d7efa54')
new_mbtest_ids <- '5e266c4629b2ac000d7efa54'
old_mbtest_ids <- '5e26299429b2ac000c57591d'
...
# Runtime changes
res_table[variable %in% 'Total_Time_P1_Hours' & abs(diff)>0.25 & abs(pct_diff)>0.25,][order(Filename, main_task, sample_round),]
