...
old_mbtest_ids <- c(
  '5d41b4fa7347c90025f7bc08'
)
new_mbtest_ids <- c(
  '5d41b5367347c9002b97c63d'
)
...
ggplot(res[variable %in% plot_vars & !is.na(diff) & Y_Type %in% c('Multiclass', 'Binary'),], aes_string(x=old_release, y=new_release, color='Y_Type')) + 
  geom_point() + geom_abline(slope=1, intercept=0) + 
  facet_wrap(~variable, scales='free') + 
  theme_bw() + theme_tufte() + ggtitle(paste(test, old_release, 'vs', new_release, 'non time series results - classification only'))


dat[Filename=='SLC14_2_dirty.csv',list(main_task, `_tasks`, run, Max_RAM_GB)][order(run, Max_RAM_GB),]
